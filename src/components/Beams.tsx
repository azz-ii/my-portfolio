...
function extendMaterial(
  ...
  const baseDefines =
    (physical as THREE.ShaderLibShader & { defines?: Record<string, unknown> })
      .defines ?? {};
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, uniformValue]) => {
    if (
      ...
      uniforms[key] = uniformValue as THREE.IUniform;
      return;
    }
    uniforms[key] = { value: uniformValue };
  });
...
function MergedPlanes({ material, width, count, height }: PlaneNoiseProps) {
  ...
  useFrame((_state, delta) => {
    ...
    const timeUniform = currentMesh.material.uniforms.time;
    if (!timeUniform) return;

    const timeValue =
      typeof timeUniform.value === "number" ? timeUniform.value : 0;
    timeUniform.value = timeValue + 0.1 * delta;
  });

  return <mesh ref={mesh} geometry={geometry} material={material} />;
}

function PlaneNoise(props: PlaneNoiseProps) {
  return (
    <MergedPlanes
...
function DirLight({ position, color }: DirLightProps) {
  ...
  useEffect(() => {
    const currentLight = dir.current;
    if (!currentLight) return;

    const cam = currentLight.shadow.camera;
    ...
    cam.right = 24;
    cam.far = 64;
    currentLight.shadow.bias = -0.004;
  }, []);
